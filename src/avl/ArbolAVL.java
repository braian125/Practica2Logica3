...
public class ArbolAVL {
    ...
    public NodoAVL insertarDato(Comparable dato, String s) {

        System.out.println("dato " + dato);
        NodoAVL nodoAInsertar = new NodoAVL(dato, s);
        if (root == null) {
            root = nodoAInsertar;
            return root;
        }
    ...
    public NodoAVL modificar(Comparable dato, String s) {
        NodoAVL aux = root;
        while (aux != null) {
            if (aux.getDato().compareTo(dato) == 0) {
                aux.setSignificado(s);
                return aux;
            } else if (aux.getDato().compareTo(dato) < 0) {
                if (aux.getLd() != null) {
                    aux = (NodoAVL) aux.getLd();
